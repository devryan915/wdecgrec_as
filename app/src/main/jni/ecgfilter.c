...
jintArray Java_com_broadchance_utils_FilterUtil_getECGDataII(JNIEnv* env,
	...
	for (nIdx = 0; nIdx < jsLen; nIdx++) {
		short v = (buf_out[nIdx * ECG_WORD_SIZE] << 8)
				| buf_out[nIdx * ECG_WORD_SIZE + 1];
...
jintArray Java_com_broadchance_utils_FilterUtil_getECGDataV1(JNIEnv* env,
	...
	nSum = malloc(sizeof(jint) * (jsLen + 1));
	for (nIdx = 0; nIdx < jsLen; nIdx++) {
		nSum[nIdx] = (buf_out[nIdx * ECG_WORD_SIZE] << 8)
				| buf_out[nIdx * ECG_WORD_SIZE + 1];
	}
...
void Java_com_broadchance_utils_FilterUtil_resetFilterR(JNIEnv* env,
		jobject obj) {
	if (hnd_l2_r)
		ecg_close(hnd_l2_r);
	hnd_l2_r = ecg_init(0);
